...
> ⚠️ **Внимание: бот был взломан.** Взлом обнаружен 13 июля 2026 года. Как только создатель бота (я) это обнаружил, бот был немедленно удалён через BotFather.
...
7. [Важное замечание о нестабильности источников](#важное-замечание-о-нестабильности-источников)
...
3. **Настройка секретов**
   ...
   Рекомендуемый способ — переменные окружения (используются в первую очередь):

   ```bash
   export BOT_TOKEN="ваш_токен"
   export KINOPOISK_API_KEY="ваш_ключ"
   export KODIK_TOKEN="ваш_токен_kodik"   # необязательно
   ```

   Для локальной разработки можно скопировать `bot/config.example.json` в `bot/config.json` и указать значения там — этот файл добавлен в `.gitignore` и не должен попадать в git.

   ⚠️ **Никогда не коммитьте реальные токены и ключи API в репозиторий**, даже во временных коммитах — история git сохраняется навсегда, а публичный репозиторий делает секреты видимыми всем. Если токен всё же был закоммичен — считайте его скомпрометированным и немедленно отзовите/пересоздайте его (для Telegram-бота — через @BotFather, `/revoke`).
...
## Важное замечание о нестабильности источников

Основной минус данного бота заключается в том, что он использует сторонние источники для поиска фильмов и сериалов (например, lordserial и goodfilms), которые могут быть нестабильными. Если один из этих сайтов меняет свою структуру или становится недоступен, бот не сможет предоставлять результаты, и разработчику придется искать новые источники или адаптировать парсеры к изменениям. Это ограничение связано с особенностью метода получения данных напрямую с веб-сайтов.
